...
public class UINavigator extends Navigator {
    ...
    public UINavigator(final ComponentContainer container) {
        ...
        if (TRACKER_ID != null && host.endsWith(appBundle("HOST"))) {
    ...
    private void initGATracker(final String trackerId) {
        tracker = new GoogleAnalyticsTracker(trackerId, appBundle("HOST"));
    ...
    private void initViewChangeListener() {
        addViewChangeListener(new ViewChangeListener() {
            ...
            public void afterViewChange(final ViewChangeEvent event) {
                DashboardViewType view = DashboardViewType.getByViewNavigName(event.getViewName());
    ...
    private void initViewProviders() {
        ...
        for (final DashboardViewType viewType : DashboardViewType.values()) {
            ViewProvider viewProvider = new ClassBasedViewProvider(
                    viewType.getViewNavigName(), viewType.getViewClass()) {
                ...
                @Override
                public View getView(final String viewNavigName) {
                    View result = null;
                    if (viewType.getViewNavigName().equals(viewNavigName)) {
                        if (viewType.isStateful()) {
                            // Stateful views get lazily instantiated
                            if (cachedInstance == null) {
                                cachedInstance = super.getView(viewType.getViewNavigName());
                            }
                            ...
                            result = super.getView(viewType.getViewNavigName());
                        }
                    }
                    return result;
                }
        ...
        setErrorProvider(new ViewProvider() {
            ...
            public String getViewName(final String viewAndParameters) {
                return ERROR_VIEW.getViewNavigName();
            }

            @Override
            public View getView(final String viewName) {
                return errorViewProvider.getView(ERROR_VIEW.getViewNavigName());
            }
        });
    }
}
