...
public final class MovieDetailsWindow extends Window {
    ...
    private Component buildFooter() {
        ...
        Button ok = new Button(appBundle("APP.CLOSE"));
        ok.addStyleName(ValoTheme.BUTTON_PRIMARY);
        ok.addClickListener((final ClickEvent event) -> {
            close();
        });
        ok.focus();
    ...
    private Component buildDetailsForm(final Movie movie, final Date startTime, final Date endTime) {
        ...
        if (startTime != null) {
            ...
            label.setCaption(appBundle("APP.DATE"));
            fields.addComponent(label);

            df.applyPattern("hh:mm a");
            label = new Label(df.format(startTime));
            label.setSizeUndefined();
            label.setCaption(appBundle("APP.STARTS"));
            fields.addComponent(label);
        }
        ...
        if (endTime != null) {
            ...
            label.setSizeUndefined();
            label.setCaption(appBundle("APP.ENDS"));
            fields.addComponent(label);
        }
