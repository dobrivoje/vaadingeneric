...
import ui.views.reports.ReportEditor.PaletteItemType;
import ui.views.reports.ReportEditor.ReportEditorListener;
...
public final class ReportsView extends TabSheet implements View, CloseHandler,
    ...
    public void onTabClose(final TabSheet tabsheet, final Component tabContent) {
        Label message = new Label(appBundle("UnsavedChangesMsg"));
        message.setWidth("25em");

        final Window confirmDialog = new Window(appBundle("UnsavedChangesButton"));
        ...
        Button ok = new Button(appBundle("Save"), (final ClickEvent event) -> {
            confirmDialog.close();
            removeComponent(tabContent);
            DashboardEventBus.post(new ReportsCountUpdatedEvent(getComponentCount() - 1));

            Notification.show(
                    uiBundle("ReportsViewDraftNotifTitle"),
                    uiBundle("ReportsViewDraftNotifMsg"),
                    Type.TRAY_NOTIFICATION
            );
        });
        ok.addStyleName(ValoTheme.BUTTON_PRIMARY);

        Button discard = new Button(appBundle("DiscardChanges"),
        ...
        Button cancel = new Button(appBundle("Cancel"), (final ClickEvent event) -> {
            confirmDialog.close();
        });
