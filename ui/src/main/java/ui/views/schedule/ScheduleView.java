...
public final class ScheduleView extends CssLayout implements View {
    ...
    private void injectMovieCoverStyles() {
        ...
        String styles = "";
        for (Movie m : MainUI.getDataProvider().getMovies()) {
    ...
    private Component buildCalendarView() {
        ...
        calendarLayout.setCaption(appBundle("Calendar"));
        ...
        calendar.setHandler((final EventClick event) -> {
    ...
    private Component buildCatalogView() {
        ...
        catalog.setCaption(appBundle("Catalog"));
        catalog.addStyleName("catalog");

        for (final Movie movie : MainUI.getDataProvider().getMovies()) {
    ...
    private Component buildTray() {
        final HorizontalLayout tray2 = new HorizontalLayout();
        tray2.setWidth(100.0f, Unit.PERCENTAGE);
        tray2.addStyleName("tray");
        tray2.setSpacing(true);
        tray2.setMargin(true);

        Label warning = new Label(uiBundle("ScheduleView.ChangeMsg"));
        warning.addStyleName("warning");
        warning.addStyleName("icon-attention");
        tray2.addComponent(warning);
        tray2.setComponentAlignment(warning, Alignment.MIDDLE_LEFT);
        tray2.setExpandRatio(warning, 1);

        ClickListener close = (final ClickEvent event) -> {
            setTrayVisible(false);
        };

        Button confirm = new Button(appBundle("Confirm"));
        confirm.addClickListener(close);
        tray2.addComponent(confirm);
        tray2.setComponentAlignment(confirm, Alignment.MIDDLE_LEFT);

        Button discard = new Button(appBundle("Discard"));
        discard.addStyleName(ValoTheme.BUTTON_DANGER);
        discard.addClickListener(close);
        discard.addClickListener((final ClickEvent event) -> {
            calendar.markAsDirty();
        });
        tray2.addComponent(discard);
        tray2.setComponentAlignment(discard, Alignment.MIDDLE_LEFT);
        return tray2;
    }
    ...
    private class MovieEventProvider implements CalendarEventProvider {
        ...
        public List<CalendarEvent> getEvents(final Date startDate, final Date endDate) {
            ...
            // when needed.
            Collection<Transaction> transactions = MainUI
                    .getDataProvider().getTransactionsBetween(startDate,
                            endDate);
            List<CalendarEvent> result = new ArrayList<>();
            for (Transaction transaction : transactions) {
                Movie movie = MainUI.getDataProvider().getMovie(
